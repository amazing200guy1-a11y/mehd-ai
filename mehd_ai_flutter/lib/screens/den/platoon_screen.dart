...
class PlatoonScreen extends StatefulWidget {
  const PlatoonScreen({super.key});

  @override
  State<PlatoonScreen> createState() => _PlatoonScreenState();
}

class _PlatoonScreenState extends State<PlatoonScreen> with SingleTickerProviderStateMixin {
  ...
  final List<Map<String, dynamic>> _generals = [
    ...
    {"rank": 3, "name": "Shadow Node 0x9", "winRate": 89.5, "pnl": 850100.25},
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              Text("PLATOON COMMUNITY NETWORK", style: MehdAiTheme.headline.copyWith(fontSize: 14, color: MehdAiTheme.gold)),
            ],
          ),
        ),
      ),
                    ...
                    return Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          // Output Shadow Button
                          _buildShadowButton(symbol, isBuy ? "LONG" : "SHORT", latestPrice),
                        ],
                      ),
                    );
                  },
  ...
  Widget _buildShadowButton(String symbol, String direction, double entryPrice) {
    return GestureDetector(
      onTap: () {
        context.read<TradingController>().executeShadowTrade(symbol, direction, entryPrice);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Shadow Trade queued for execution.", style: MehdAiTheme.terminalStyle.copyWith(color: MehdAiTheme.text(context))),
      ...
      child: Container(
        ...
        child: Text("SHADOW", style: MehdAiTheme.terminalStyle.copyWith(color: MehdAiTheme.purple, fontWeight: FontWeight.bold)),
      ),
    );
  }
}
