...
/// FILE 8 — tutorial_screen.dart
///
/// Build Debrief:
/// Swipeable tutorial cards teach 3 concepts before the user sees a chart.
/// The final button sets onboardingComplete = true in Firestore.
...
class _TutorialScreenState extends State<TutorialScreen> {
  ...
  Future<void> _enterApp() async {
    ...
    if (userId != null) {
      try { await userService.completeOnboarding(userId); } catch (_) {}
    }
    if (mounted) {
              ...
              child: Row(
                ...
                children: List.generate(3, (i) => AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    color: _currentPage == i ? MehdAiTheme.blue : MehdAiTheme.borderColor,
  ...
  Widget _card2() => _buildCard(
    icon: Icons.auto_graph, color: MehdAiTheme.green, title: 'The Zen Chart',
    subtitle: 'No cluttered indicators', body: 'The AI paints exactly where to look',
    details: ['🟢  Green zones — AI sees buying pressure', '🔴  Red zones — AI sees selling pressure', '🟡  Yellow zones — wait for confirmation'],
    footer: 'Clean, focused, and driven by The Den',
  );

  Widget _card3() => _buildCard(
    icon: Icons.shield, color: MehdAiTheme.purple, title: 'The Risk Kernel',
    subtitle: 'Your money is protected by rules no AI can break', body: '',
    details: ['✅  Max 1% risk per trade — always', '✅  Kill switch at 3% daily loss', '✅  24-hour lock after kill switch', '✅  No AI can override risk limits'],
    footer: 'You are now ready to trade safely',
  );
  ...
  Widget _buildCard({required IconData icon, required Color color, required String title,
    ...
    return SingleChildScrollView(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const SizedBox(height: 24),
        Text(title, style: MehdAiTheme.headingStyle.copyWith(fontSize: 26, fontWeight: FontWeight.w700), textAlign: TextAlign.center),
        const SizedBox(height: 12),
        Text(subtitle, style: MehdAiTheme.labelStyle.copyWith(fontSize: 15, height: 1.4), textAlign: TextAlign.center),
